...
@Component({
  selector: 'app-trade-insurance-photo',
  templateUrl: './trade-insurance-photo.component.html',
  styleUrls: ['./trade-insurance-photo.component.scss']
})
export class TradeInsurancePhotoComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.queryService.httpGet(`${api_path.getMerchantInfoWithId}/${id}`, header).subscribe(res =>{
      console.log(res);
      this.data = res;
      this.imgSrc = `data:image/png;base64,${this.data.tradeInsuranceFile}`;
    });
  }
